...
class SaveToMysql:
    ...
    def insert_data(self, ip, mac, memory, cpu, device_name, os_version, flash, is_success, error_msg, data_txt):
        """save files and os data"""
        print(ip, memory, cpu, error_msg, data_txt)
        try:
            sql = f"""
                insert into save_files(`ip`,`mac`, `memory`, `cpu`,`device_name`,`os_version`,`flash`,`is_success`,
                `error_msg`,`file_data`, `create_time`, `update_time`) values ( %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);
            """
            create_time = datetime.datetime.now()
            update_time = datetime.datetime.now()
            values = (
                ip, mac, memory, cpu, device_name, os_version, flash, is_success, error_msg, data_txt, create_time,
                update_time)
